...
const Dashboard: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    const producedToday = factoryPieces.filter(p => p && p.lastUpdated?.startsWith(today) && (p.status === 'Cut' || p.status === 'Tempered')).length;
    const pendingDispatch = factoryPieces.filter(p => p && p.status === 'Ready to Dispatch').length;
