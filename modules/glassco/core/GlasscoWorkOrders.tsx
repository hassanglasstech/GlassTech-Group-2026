...
const STATUS_TONE: Record<string, string> = {
  ...
  'In-Progress':'bg-primary-subtle text-primary-hover border-primary-border',
  Completed:   'bg-emerald-50 text-emerald-700 border-emerald-200',
  Cancelled:   'bg-rose-50 text-rose-600 border-rose-200',
};
const PRIORITY_TONE: Record<string, string> = {
  Low:    'bg-slate-100 text-slate-500',
  Normal: 'bg-primary-subtle text-primary-hover',
...
const GlasscoWorkOrders: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    await AsyncSalesService.saveWorkOrders([row as SbLooseRow]);
  ...
  const handleQuickStatus = async (wo: WorkOrder, status: string) => {
    ...
    await AsyncSalesService.saveWorkOrders([updated as SbLooseRow]);
    await refresh();
  };
    ...
    <div className="space-y-4 animate-in fade-in duration-300">
      <div className="bg-gradient-to-br from-blue-700 to-cyan-700 text-white p-5 rounded-2xl flex items-center justify-between shadow-xl">
        ...
        <button onClick={() => setEditing(blank(company))}
          className="bg-white text-primary-hover px-4 py-2 rounded-xl text-xs font-black uppercase hover:bg-primary-subtle shadow flex items-center gap-2"
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {visible.map(w => (
              <tr key={w.id} className="hover:bg-slate-50">
                <td className="px-4 py-3 font-black text-primary-hover">{w.id}</td>
                ...
                <td className="px-4 py-3 text-right">
                  <button onClick={() => setEditing({ ...w })} className="text-[10px] font-bold text-primary hover:underline mr-2">Edit</button>
      ...
      {editing && (
        <div className="fixed inset-0 bg-slate-900/60 z-[500] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="bg-primary-hover text-white px-5 py-3 flex items-center justify-between">
            ...
            <div className="px-5 py-3 bg-slate-50 border-t flex justify-end gap-2">
              ...
              <button onClick={handleSave} className="px-4 py-2 bg-primary-hover text-white rounded-lg text-xs font-black uppercase hover:bg-blue-800 flex items-center gap-1.5"><Save size={12}/> Save</button>
            </div>
          </div>
        </div>
      )}
