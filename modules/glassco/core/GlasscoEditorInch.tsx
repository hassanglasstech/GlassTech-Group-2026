...
export const GlasscoEditorInch: React.FC<Props> = ({
    ...
    // FIX: Backfill thickness from description for legacy products without explicit thickness
    ...
    const getThicknesses = (category: string, subCategory: string) => {
        const filtered = glassMaster.filter(p => 
            (p.glassType === category || (p as any).glass_type === category) && 
            (p.subCategory === subCategory || (p as any).sub_category === subCategory || (subCategory === 'Standard' && !p.subCategory && !(p as any).sub_category))
        );
    ...
    const isWastageZone = (item: QuotationItem) => {
        ...
        const w = (Number(item.inchW) || 0) + ((Number(item.sootW) || 0) / 8);
        const h = (Number(item.inchH) || 0) + ((Number(item.sootH) || 0) / 8);
        // Alert if Width >= 55" or Height >= 115"
        return w >= 55 || h >= 115;
    };
        ...
        <div className="bg-white rounded-3xl w-full h-[95vh] shadow-2xl flex flex-col overflow-hidden border border-slate-300 no-print">
            <div className="sap-object-header flex justify-between items-center py-4 px-6 bg-white border-b shrink-0">
                <div className="flex items-center space-x-4">
                    ...
                    <h3 className="text-xl font-black uppercase text-slate-800 tracking-tight">Inch Configurator</h3>
                </div>
                <div className="flex space-x-3">
                    <button onClick={onSwitchMode} className="px-4 py-2 rounded-xl text-xs font-black uppercase border bg-white text-slate-600 border-slate-300 flex items-center hover:bg-slate-50 transition-all">
                        <ArrowRightLeft size={14} className="mr-2"/> Switch to MM
                    </button>
            ...
            <div className="flex-1 overflow-hidden p-6 bg-[#f8fafc] flex flex-col space-y-4">
                <div className="bg-white p-5 rounded-2xl border grid grid-cols-12 gap-3 items-end shadow-sm shrink-0">
                    ...
                    <div className="col-span-1 space-y-1">
                        ...
                        <input type="date" className="sap-input w-full font-bold h-10 border-slate-300 text-xs" value={formData.date} onChange={e => onUpdateItem(-1, 'date', e.target.value)} />
                    </div>
                    <div className="col-span-1 space-y-1 relative group">
                        ...
                        <input 
                            ...
                            className="sap-input w-full font-black h-10 border-indigo-200 bg-indigo-50 placeholder:text-indigo-200" 
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead className="sticky top-0 z-10 bg-slate-50 border-b">
                                <tr className="text-[10px] uppercase font-black text-slate-400 tracking-widest">
                                    ...
                                    <th className="w-36 text-center py-4">Width (In.St)</th>
                                    <th className="w-36 text-center py-4">Height (In.St)</th>
                                    ...
                                    return (
                                        <tr key={idx} className={`${item.isSection ? 'bg-slate-50' : 'hover:bg-blue-50/20'}`}>
                                            ...
                                            <td className="py-2 px-2">
                                                {item.isSection ? (
                                                    <input type="text" className="w-full bg-transparent font-black uppercase text-blue-700 outline-none text-xs h-8 border-b border-blue-100 focus:border-blue-500" value={item.description} onChange={e => onUpdateItem(idx, 'description', e.target.value)} placeholder="SECTION HEADING..." />
                                                ) : (
                                            ...
                                            <td className="px-1"><div className="flex justify-center gap-0.5 items-center"><input type="number" className={`${stdInput} w-16`} value={item.inchW} onChange={e => onUpdateItem(idx, 'inchW', e.target.value)}/><span className="font-black text-slate-400">.</span><input type="number" className={`${stdInput} w-10 text-slate-400`} value={item.sootW} onChange={e => onUpdateItem(idx, 'sootW', e.target.value)} max="7" placeholder="0"/></div></td>
                                            <td className="px-1"><div className="flex justify-center gap-0.5 items-center"><input type="number" className={`${stdInput} w-16`} value={item.inchH} onChange={e => onUpdateItem(idx, 'inchH', e.target.value)}/><span className="font-black text-slate-400">.</span><input type="number" className={`${stdInput} w-10 text-slate-400`} value={item.sootH} onChange={e => onUpdateItem(idx, 'sootH', e.target.value)} max="7" placeholder="0"/></div></td>
