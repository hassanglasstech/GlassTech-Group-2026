...
// Minimal typed shape this print reads off each PO line (PO items are loosely typed).
    ...
    return (
        <div id="glassco-service-order-root" className="glassco-service-order-page bg-white text-black p-0 font-sans leading-tight min-h-screen flex flex-col">
            {/* this print renders STANDALONE (not via GlasscoPrintTemplate),
                ...
                own print stylesheet. Now everything is scoped to a unique root id
                (#glassco-service-order-root) — visibility is toggled via that id only,
                and table/page rules are namespaced under it so no global selector leaks. */}
            <style>{`
                @media screen { #glassco-service-order-root { display: none !important; } }
                @media print {
                    ...
                    /* SHOW PRINT CONTAINER (scoped to this root only) */
                    #glassco-service-order-root, #glassco-service-order-root * { visibility: visible; }
                    #glassco-service-order-root {
                        display: block !important;
                        position: static !important;
                        width: 100% !important;
                        background: white !important;
                    }
                    #glassco-service-order-root table { page-break-inside: auto; }
                    #glassco-service-order-root thead { display: table-header-group; }
                    #glassco-service-order-root tr { page-break-inside: avoid; }
                    #glassco-service-order-root .print-container { width: 100% !important; padding: 15mm !important; box-sizing: border-box !important; }
                    #glassco-service-order-root .font-pill-service { border: 2px solid #e11d48; border-radius: 9999px; padding: 6px 50px; font-weight: 900; letter-spacing: 0.2em; color: #e11d48; }
                    #glassco-service-order-root .page-break-before { page-break-before: always; }
                }
            `}</style>
            ...
            <div className="print-container">
                ...
                <div className="flex-1 mt-4">
                    {(() => {
                        ...
                        const groupedItems: { orderId: string, items: ServiceLine[] }[] = [];
                        const orderMap = new Map<string, ServiceLine[]>();

                        po.items.forEach((raw) => {
                            ...
                            const oId = (r.orderId as string) || 'General';
                            if (!orderMap.has(oId)) {
                                orderMap.set(oId, []);
                            }
