...
export const GlassCoJobCardPrint: React.FC<Props> = ({ quote, clientName, pieces, products }) => {
    ...
    // Generate virtual pieces for preview if none found
    ...
    // Flatten sections and pieces into printable rows
    ...
    return (
        <div className="glassco-print-page bg-white text-black font-sans leading-tight">
            <table className="w-full text-left border-collapse text-xs" style={{ tableLayout: 'fixed' }}>
                ...
                {/* ═══ TBODY: ALL pieces — zero chunking ═══ */}
                <tbody>
                    {printableItems.map((item, idx) => {
                        if (item.isSection) {
                            return (
                                <tr key={`sec-${idx}`} style={{ background: '#f1f5f9' }}>
                                    <td colSpan={6} style={{ padding: '6px', border: '2px solid black', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.15em', textAlign: 'center', fontStyle: 'italic', fontSize: '12px' }}>
                                        {item.description}
                                    </td>
                                </tr>
                            );
            ...
            {/* ═══ FOOTER: Signatures — once after all pieces ═══ */}
