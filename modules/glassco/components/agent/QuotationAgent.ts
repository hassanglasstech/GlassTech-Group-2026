...
function executeTool(
  ...
  switch (toolName) {
    ...
    // ── save_quotation ────────────────────────────────────────────────
    case 'save_quotation': {
      // Guard (mirror of the human save path in useGlasscoQuotations): never let
      // the agent persist an empty or zero-value quotation. The tool schema marks
      // items as required, but a confused model can still call it with [] / 0 — and
      // a junk Draft pollutes the sales pipeline and burns a serial number.
      const incomingItems = (params.items || []) as QuotationItem[];
      const realLineCount = incomingItems.filter((i) => !i.isSection).length;
      const incomingTotal =
        incomingItems.reduce((s, i) => s + (Number(i.amount) || 0), 0) +
        ((params.serviceCharges || []) as GlassServiceCharge[]).reduce((s, sc) => s + (Number(sc.amount) || 0), 0);
      if (realLineCount === 0 || incomingTotal <= 0) {
        return { error: 'Cannot save an empty quotation — add at least one line item with an amount > 0 before saving.' };
      }
      ...
      const newQuotation: Quotation = {
        ...
        // Approval/sending stays a manual, human action in the Sales UI. Do not
        // add a status param to this tool or read params.status here.
        status:            'Draft' as any,
      ...
      AsyncSalesService.saveQuotations([newQuotation]).catch(() => { /* queued for retry */ });
...
export async function generateQuotation(
  ...
  // by the other ERP agents (EventClassifier, MultiAgentOrchestrator, etc.).
  const safeRequest = sanitizeUserInput(userRequest);
  const messages: ClaudeMessage[] = [
    { role: 'user', content: safeRequest },
  ];
