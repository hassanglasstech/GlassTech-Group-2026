...
// IMPORT FIX: Aapki file ka sahi naam 'supabaseClient.ts' hai
...
const NipponProductMaster = () => {
  ...
  // REFRESH FIX: Page load ya refresh par data database se uthana
  useEffect(() => {
    fetchProducts();
  }, []);
  ...
  // SEARCH FIX: Name aur Item Code dono include kar diye hain
    ...
    <div className="p-6 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="bg-blue-50 px-6 py-2 rounded-2xl border border-blue-100 text-center min-w-[120px]">
          <p className="text-[9px] font-black uppercase text-blue-400">Total Items</p>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-32 space-y-4">
          ...
          <p className="text-xs font-black uppercase text-slate-400 tracking-widest">Syncing with Database...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredProducts.map((product) => (
            <div key={product.id} className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden hover:shadow-xl transition-all group flex flex-col">
              ...
              <div className="h-52 bg-slate-50 relative flex items-center justify-center overflow-hidden border-b">
                {product.image_url ? (
                  ...
                  <div className="flex flex-col items-center text-slate-200">
                    ...
                    <span className="text-[9px] font-black mt-2 uppercase">Image Missing</span>
                  </div>
                )}
              ...
              <div className="p-6 flex-1 flex flex-col justify-between">
                ...
                <button className="w-full py-3 bg-slate-50 border border-slate-200 text-slate-600 rounded-2xl text-[10px] font-black uppercase hover:bg-slate-900 hover:text-white hover:border-slate-900 transition-all shadow-sm">
                  View Specifications
                </button>
              </div>
            </div>
      ...
      {!loading && filteredProducts.length === 0 && (
        <div className="bg-white py-24 text-center rounded-[3rem] border-2 border-dashed border-slate-200">
          ...
          <h4 className="text-slate-400 font-black uppercase tracking-widest">No matching hardware found</h4>
          <p className="text-slate-300 text-[10px] font-bold mt-2 uppercase italic">Try searching with a different name or part code</p>
        </div>
      )}
    </div>
  );
