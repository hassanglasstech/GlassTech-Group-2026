...
const GlasscoProductMaster: React.FC<{ company: Company }> = ({ company }) => {
  ...
  return (
    <div className="flex h-[calc(100vh-100px)] gap-6 animate-in fade-in duration-300">
      <div className="w-64 flex flex-col space-y-2 shrink-0">
        {menuItems.map(item => (
            <button key={item.id} onClick={() => setActiveTab(item.id as any)} className={`p-4 rounded-2xl text-left border-2 transition-all flex items-center gap-3 ${activeTab === item.id ? 'bg-slate-800 text-white border-slate-800 shadow-lg' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'}`}>
            <item.icon size={20} className={activeTab === item.id ? 'text-orange-400' : 'text-slate-400'} />
        ...
        {activeTab === 'Systems' && (
          <div className="mt-6 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-hidden flex-1">
            <div className="p-2 border-b grid grid-cols-3 gap-1">{appNavItems.map(nav => (<button key={nav.id} onClick={() => { setNavSection(nav.id as any); setSelectedSystem(null); }} className={`flex flex-col items-center justify-center p-2 rounded-lg transition-all ${navSection === nav.id ? 'bg-orange-50 text-orange-600' : 'text-slate-400 hover:bg-slate-50'}`}><nav.icon size={16} /><span className="text-[8px] font-black uppercase mt-1">{nav.label}</span></button>))}</div>
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-4">
              {['Thermal', 'Non-Thermal'].map(subClass => {
                  ...
                  return (
                      <div key={subClass}><p className="px-2 text-[9px] font-black uppercase text-blue-400 mb-1">{subClass}</p>{subSystems.map(s => (<button key={s} onClick={() => setSelectedSystem(s)} className={`w-full text-left px-4 py-2 mb-1 rounded-lg text-xs font-bold transition-all ${selectedSystem === s ? 'bg-orange-500 text-white shadow-md' : 'text-slate-600 hover:bg-slate-50'}`}>{s}</button>))}</div>
                  )
              })}
            </div>
            <div className="p-2 border-t"><button onClick={() => { setSelectedSystem(''); setIsModalOpen(true); }} className="w-full py-2 border-2 border-dashed border-slate-300 rounded-xl text-slate-400 font-bold text-xs uppercase hover:border-orange-400 hover:text-orange-500 transition-all">+ New System</button></div>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 bg-white rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col overflow-hidden">
        <div className="px-8 py-6 border-b flex justify-between items-center bg-slate-50/50">
          <div><h2 className="text-2xl font-black uppercase text-slate-800">{activeTab === 'Systems' ? (selectedSystem || 'Select System') : `${activeTab} Master`}</h2><div className="flex items-center space-x-2 mt-1">{activeTab === 'Systems' && selectedSystem && (<span className="px-2 py-0.5 rounded bg-blue-100 text-blue-700 text-[9px] font-black uppercase">{getSystemCategory(selectedSystem)}</span>)}<p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Manage Costing & Recipe Data</p></div></div>
          <button onClick={() => { resetForms(); setIsModalOpen(true); }} className="bg-slate-900 text-white px-6 py-3 rounded-xl font-black uppercase text-xs tracking-widest flex items-center gap-2 shadow-xl hover:bg-blue-600 transition-all" disabled={activeTab === 'Systems' && !selectedSystem && systems.length > 0}><Plus size={16}/> <span>Add {activeTab} Item</span></button>
        </div>
