...
const JobCard: React.FC<JobCardProps> = ({ piece, jobOrder, spot, onBinClick, actionRenderer }) => {
  ...
  const orderNumeric = piece.orderId.split('-').filter(p => !p.startsWith('R')).pop() || '';
  const pieceNumber = piece.id.includes('/') ? piece.id.split('/').pop()?.split('-')[0] : '';
  const displayId = `${orderNumeric}/${pieceNumber}`;
