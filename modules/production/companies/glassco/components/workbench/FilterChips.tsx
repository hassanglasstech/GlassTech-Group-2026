...
const Chip: React.FC<ChipProps> = ({ icon, label, value, options, onSelect }) => {
  ...
  // A11y: focus the first option when the menu opens (keyboard entry point).
  useEffect(() => {
    if (open) menuRef.current?.querySelector<HTMLButtonElement>('button')?.focus();
  }, [open]);

  // A11y: arrow-key roving focus + Escape-to-close on the option list.
  ...
  return (
    <div ref={ref} className="relative inline-flex">
      ...
      {open && (
        <div ref={menuRef} role="listbox" aria-label={`${label} options`} onKeyDown={onMenuKeyDown} className="absolute top-full mt-1 left-0 z-dropdown bg-white rounded-lg shadow-xl border border-slate-200 min-w-[180px] py-1 max-h-64 overflow-y-auto">
          {options.map(opt => (
            <button
              key={opt.value}
              type="button"
