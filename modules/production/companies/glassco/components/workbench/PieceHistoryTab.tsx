        ...
        <li key={row.id} className="relative">
          ...
          <div className="bg-white border border-slate-200 rounded-lg p-2.5 shadow-sm">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] font-black uppercase text-slate-400">{row.operation}</span>
              <span className="text-[10px] text-slate-400">{relativeTime(row.changed_at)}</span>
            </div>
            <div className="text-xs">{diffSummary(row)}</div>
            {row.changed_by && (
              <div className="text-[10px] text-slate-400 mt-1">by {row.changed_by}</div>
            )}
          </div>
        </li>
      ))}
