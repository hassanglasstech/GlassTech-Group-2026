  ...
  return (
    ...
    >
      ...
      <div className="px-2.5 py-2 flex items-center gap-2">
        ...
        <span
          role="img"
          aria-label={`Piece age: ${days} day${days === 1 ? '' : 's'} since last update`}
      ...
      {density !== 'compact' && (
        <div className="px-2.5 pb-2 flex items-center justify-between gap-2 -mt-1">
          <span className="text-2xs text-slate-500 font-bold truncate">{piece.orderId}</span>
      ...
      {density === 'detailed' && (
        <div className="px-2.5 pb-2 space-y-1 border-t border-slate-100 pt-2">
          {piece.specs && (
            <p className="text-2xs text-slate-500 line-clamp-2">{piece.specs}</p>
          )}
      ...
      <input
        ...
        className={`
          ...
          ${inSelectionMode || selected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 focus:opacity-100'}
