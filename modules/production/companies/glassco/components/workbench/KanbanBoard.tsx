  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-end gap-2 mb-3">
        <span className="text-2xs font-bold uppercase text-slate-400">Density</span>
  ...
  return (
    ...
    >
      ...
      <div className="px-1 pb-2 flex items-center justify-between border-b border-slate-200">
        ...
        <span className="text-2xs font-black px-1.5 py-0.5 rounded-full bg-white text-slate-600">
          {pieces.length}
        </span>
      </div>
      ...
      >
        <div className="flex-1 overflow-y-auto pt-2 pb-1 space-y-1.5 px-0.5">
          {pieces.length === 0 ? (
            <div className="text-center py-6 text-2xs text-slate-400 italic">
          ...
          {pieces.length > limit && (
            <button
              ...
              className="w-full text-2xs font-bold text-blue-600 hover:text-blue-700 hover:bg-blue-50 rounded py-1.5 transition-colors"
            >
              Show {Math.min(100, pieces.length - limit)} more ({pieces.length - limit} hidden)
            </button>
          )}
        </div>
      </SortableContext>
