...
const PieceDetailPanel: React.FC<PieceDetailPanelProps> = ({
  ...
  return (
    <>
      ...
      <div
        className="fixed inset-0 z-panel bg-slate-900/30 md:bg-slate-900/20 transition-opacity"
        onClick={onClose}
        aria-hidden
      />
      ...
      <aside
        className={`
          fixed z-panel bg-white shadow-2xl flex flex-col
          inset-x-0 bottom-0 max-h-[88vh] rounded-t-card   /* mobile: bottom sheet */
          md:inset-y-0 md:right-0 md:left-auto md:bottom-auto md:max-h-none md:rounded-none md:rounded-l-card
          ...
          <div className="flex items-center gap-1 shrink-0">
            {prevPiece && (
              <button
                ...
                title="Previous (←)"
                aria-label="Previous piece (Left arrow)"
              >
                <ChevronLeft size={16}/>
              </button>
            )}
            {nextPiece && (
              <button
                ...
                className="p-1.5 rounded-lg text-slate-500 hover:bg-slate-100"
                title="Next (→)"
                aria-label="Next piece (Right arrow)"
              >
                ...
                {photos.map(ph => (
                  ...
                  >
                    <img
                      ...
                      loading="lazy"
                    />
                    <div className="px-2 py-1 text-2xs font-bold text-slate-600 bg-slate-50">
...
const Row: React.FC<{ label: string; children: React.ReactNode }> = ({ label, children }) => (
  <div className="flex items-baseline gap-3">
    <dt className="text-2xs font-black uppercase text-slate-400 w-20 shrink-0">{label}</dt>
