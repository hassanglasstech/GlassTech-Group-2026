...
const ShortcutSheet: React.FC = () => {
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-sheet bg-slate-900/40"
      ...
      <div
        ...
        className="fixed z-sheet inset-x-4 bottom-4 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 bg-white rounded-card shadow-2xl max-w-2xl md:w-[680px] max-h-[80vh] overflow-hidden flex flex-col"
      >
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          {GROUPS.map(g => (
            <section key={g.name}>
              <h3 className="text-2xs font-black uppercase tracking-wider text-slate-400 mb-2">
                {g.name}
              </h3>
                    ...
                    <span className="flex gap-1.5 shrink-0">
                      {s.keys.map((k, j) => (
                        <React.Fragment key={k}>
                          ...
                          <kbd className="bg-slate-100 border border-slate-200 rounded px-1.5 py-0.5 text-2xs font-mono font-bold text-slate-700">
                            {k}
                          </kbd>
                        </React.Fragment>
                      ))}
        ...
        <footer className="px-5 py-2.5 border-t border-slate-200 text-2xs text-slate-400 text-center shrink-0">
