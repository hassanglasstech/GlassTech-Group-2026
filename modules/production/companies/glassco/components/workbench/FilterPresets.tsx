...
const FilterPresets: React.FC<FilterPresetsProps> = ({ currentQuery, onApply }) => {
  ...
  const remove = async (id: string) => {
    ...
    if (!target) return;
    if (!await confirmModal(`Delete preset "${target.name}"?`)) return;
  ...
  return (
    <>
      <div className="px-3 pt-1 pb-2 mt-2 border-t border-slate-200">
        ...
        {sorted.length === 0 ? (
          <p className="text-2xs text-slate-400 italic px-1 py-1">
      ...
      {dialogOpen && (
        <>
          <div className="fixed inset-0 z-sheet bg-slate-900/40" onClick={() => setDialog(false)} aria-hidden/>
          <div
            role="dialog"
            aria-label="Save filter preset"
            className="fixed z-sheet left-1/2 top-1/3 -translate-x-1/2 bg-white rounded-xl shadow-2xl w-[360px] p-5"
          >
            ...
            />
            {currentQuery && (
              <p className="text-2xs text-slate-400 font-mono break-all mb-3">{currentQuery}</p>
            )}
