...
import {
  Clock, AlertTriangle, TrendingDown, Activity, RefreshCw,
  Truck, CheckCircle2,
} from 'lucide-react';
...
const WIPAging: React.FC = () => {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        setDispatches(ProductionService.getTemperingDispatches().filter((d: any) => d.company === company || d.company === 'Factory'));
      }
    } catch {
      setDispatches(ProductionService.getTemperingDispatches().filter((d: any) => d.company === company || d.company === 'Factory'));
    }
  ...
  const allClients = useMemo(() => SalesService.getClients(), [tick]);
  const allOrders  = useMemo(() => SalesService.getQuotations(), [tick]);
  ...
  return (
    <div className="space-y-5 p-5 max-w-7xl mx-auto">
      {/* Header */}
      <div className="bg-gradient-to-br from-rose-700 to-amber-700 text-white rounded-2xl p-6 shadow-xl flex items-center justify-between">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KpiCard label="Stuck >7d"   value={stuckPieces.filter(s => s.days >= 7  && s.days <= 14).length} tone="warn"/>
        <KpiCard label="Stuck >14d"  value={stuckPieces.filter(s => s.days > 14 && s.days <= 30).length} tone="bad"/>
        <KpiCard label="Stuck >30d"  value={stuckPieces.filter(s => s.days > 30).length} tone="critical"/>
        <KpiCard label="Vendors Overdue" value={totalOverdue} tone={totalOverdue > 0 ? 'bad' : 'good'} icon={<Truck size={12}/>} />
      </div>
      ...
      <div className="bg-white rounded-2xl border-2 border-slate-200 shadow-sm overflow-hidden">
        <div className="px-4 py-3 bg-slate-50 border-b flex items-center justify-between">
          ...
          <p className="text-[10px] text-slate-400 font-bold uppercase">{visibleStuck.length} rows · sorted by oldest first</p>
        </div>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
              {visibleStuck.length === 0 && (
                <tr><td colSpan={6} className="p-12 text-center text-slate-300 italic font-bold">No stuck pieces — clean shop floor.</td></tr>
              )}
              {visibleStuck.map(({ piece, days, order }) => {
                ...
                return (
                  <tr key={piece.id} className={`hover:bg-slate-100 ${cls.row}`}>
                    <td className="px-3 py-2">
                      <span className={`text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-wider ${cls.pill}`}>
                        {days}d
                      </span>
                    </td>
                    ...
                    <td className="px-3 py-2">
                      ...
                      {order?.projectName && <p className="text-[9px] text-slate-400 font-bold uppercase truncate">{order.projectName}</p>}
                    </td>
                    <td className="px-3 py-2 font-bold text-slate-700">{piece.status}</td>
                    <td className="px-3 py-2 text-[10px] text-slate-500 font-bold">{(piece.lastUpdated || '').replace('T', ' ').slice(0, 16)}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      <div className="bg-white rounded-2xl border-2 border-slate-200 shadow-sm overflow-hidden">
        <div className="px-4 py-3 bg-slate-50 border-b flex items-center justify-between">
          ...
          <p className="text-[10px] text-slate-400 font-bold uppercase">Migration 047 columns: expected_return_date / actual_return_date</p>
        </div>
        {vendorSLA.length === 0 ? (
          <p className="p-10 text-center text-slate-300 italic font-bold text-sm">No tempering dispatches yet.</p>
        ) : (
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 text-[9px] font-black uppercase text-slate-400 tracking-widest border-b">
              <tr>
                ...
                return (
                  <tr key={v.vendor} className="hover:bg-slate-50">
                    ...
                    <td className="px-3 py-2.5 text-right">
                      {v.overdue > 0 ? (
                        <span className="text-[10px] font-black px-2 py-0.5 rounded bg-rose-600 text-white">{v.overdue}</span>
                      ) : (
                        <span className="text-[10px] font-bold text-slate-300">—</span>
                      )}
                    </td>
                    <td className="px-3 py-2.5 text-right">
                      <span className={`text-[11px] font-black px-2 py-1 rounded ${pctTone}`}>
                        {v.onTimePct}%
                      </span>
                    </td>
                  </tr>
...
const KpiCard: React.FC<{
  label: string; value: number; tone?: 'good' | 'warn' | 'bad' | 'critical' | 'default';
  ...
  return (
    <div className={`rounded-2xl border-2 ${cls} p-4`}>
      <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest opacity-70">
        {icon || (tone === 'bad' || tone === 'critical' ? <AlertTriangle size={12}/> : tone === 'good' ? <CheckCircle2 size={12}/> : <TrendingDown size={12}/>)}
        <span>{label}</span>
      </div>
