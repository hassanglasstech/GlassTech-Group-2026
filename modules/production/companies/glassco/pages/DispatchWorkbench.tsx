    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="bg-blue-900 text-white px-4 py-3 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-2xs text-blue-200 leading-tight">{user?.fullName ?? user?.email}</p>
          </div>
        </div>
        <button
          type="button"
      ...
      <main className="flex-1 overflow-y-auto p-4 pb-24">
        {visiblePieces.length === 0 ? (
          <EmptyState
            icon={<Truck size={22}/>}
            title="No pieces ready to dispatch."
            description="QC-passed pieces will appear here automatically."
          />
        ) : (
                    ...
                    {list.map(p => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-2xs text-slate-500 truncate max-w-xs">
                            {p.specs ?? ''}
                          </span>
                        </li>
                      );
