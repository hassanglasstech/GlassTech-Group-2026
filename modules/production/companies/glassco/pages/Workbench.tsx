...
import { Search as SearchIcon, RefreshCw, Package, MapPin, Printer, HelpCircle, LayoutGrid } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-20 no-print">
        {/* Title row — makes the board's purpose explicit at a glance */}
        <div className="px-4 pt-3 flex items-center gap-2 flex-wrap">
          <LayoutGrid size={17} className="text-blue-600 shrink-0" />
          <h1 className="text-base font-black uppercase tracking-tight text-slate-800">Production Board</h1>
          <span className="text-2xs text-slate-400 font-bold hidden sm:inline">· Track every glass piece from cutting → QC → tempering → dispatch → delivery</span>
        </div>
        ...
        <div className="px-4 pb-2 text-2xs text-slate-400 flex items-center gap-2">
  ...
  <VirtualPieceGrid
    ...
    cellRenderer={(p) => (
      ...
      >
        ...
        <div className="text-2xs text-slate-400 mt-auto flex items-center gap-2">
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="bg-white rounded-xl shadow border border-slate-200 p-8 max-w-sm text-center">
          ...
          <p className="text-sm text-slate-500">
            The Production Board is only available to production roles.
