...
import {
  CheckCircle2, X, AlertTriangle, ShieldCheck, Filter, RefreshCw, Eye,
  Globe, Search,
} from 'lucide-react';
...
const QCWorkbench: React.FC = () => {
  ...
  const allPieces = useMemo<ProductionPiece[]>(() => {
    return ProductionService.getProductionPieces();
  }, [tick]);

  const jobOrders = useMemo<Quotation[]>(() => {
    const all = SalesService.getQuotations();
    return all.filter(q => q.company === company);
  }, [company, tick]);
  ...
  const updateStatusAtomic = async (pieceId: string, nextStatus: PieceStatus, extra: any = {}) => {
    ...
    ProductionService.saveProductionPieces(
    ...
    <div className="min-h-screen bg-slate-50 pb-32" style={{ fontSize: 16 }}>
      ...
      <header className="sticky top-0 z-30 bg-emerald-700 text-white px-4 py-3 shadow">
        <div className="flex items-center justify-between gap-2">
          <div className="min-w-0">
            <p className="text-[11px] text-emerald-200 font-bold uppercase tracking-widest">{t.title}</p>
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white rounded-2xl border-2 border-slate-200 p-3 text-center">
            <p className="text-[11px] font-black text-slate-400 uppercase">{t.pending}</p>
          ...
          <div className="bg-amber-50 rounded-2xl border-2 border-amber-200 p-3 text-center">
            <p className="text-[11px] font-black text-amber-700 uppercase">{t.mandatory}</p>
            <p className="text-2xl font-black text-amber-700">{items.filter(i => i.isMandatory).length}</p>
          </div>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="bg-white border-2 border-dashed border-slate-200 rounded-2xl p-12 text-center">
            ...
            <p className="text-sm font-bold text-slate-400">{t.noPending}</p>
          </div>
        ) : (
          <div className="space-y-3">
            {filtered.map(it => {
              ...
              return (
                <div key={it.piece.id} className={`bg-white rounded-2xl border-2 shadow-sm overflow-hidden ${it.isMandatory ? 'border-amber-300' : 'border-slate-200'}`}>
                  {/* Header */}
                  <div className={`px-4 py-3 border-b ${it.isMandatory ? 'bg-amber-50' : 'bg-slate-50'}`}>
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {it.isMandatory && <span className="text-[10px] font-black text-amber-700 bg-amber-100 px-2 py-0.5 rounded-full uppercase">{t.mandatory}</span>}
                    </div>
                    <p className="text-[11px] text-slate-500 font-bold mt-0.5 truncate">{it.size} · {it.thickness} · {it.piece.orderId}</p>
                  </div>
