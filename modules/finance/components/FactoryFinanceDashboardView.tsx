  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        ...
        <p className="text-3xl font-bold text-slate-900">PKR {(Number(metrics.expenses) || 0).toLocaleString()}</p>
      </div>
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h3 className="text-sm font-semibold text-slate-500 uppercase">Repair & Maintenance</h3>
        <p className="text-3xl font-bold text-amber-600">PKR {(Number(metrics.repairMaintenance) || 0).toLocaleString()}</p>
      </div>
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h3 className="text-sm font-semibold text-slate-500 uppercase">Asset Purchases</h3>
        <p className="text-3xl font-bold text-blue-600">PKR {(Number(metrics.assetPurchases) || 0).toLocaleString()}</p>
      </div>
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h3 className="text-sm font-semibold text-slate-500 uppercase">Consumable Purchases</h3>
        <p className="text-3xl font-bold text-emerald-600">PKR {(Number(metrics.consumablePurchases) || 0).toLocaleString()}</p>
      </div>
    </div>
  );
};
