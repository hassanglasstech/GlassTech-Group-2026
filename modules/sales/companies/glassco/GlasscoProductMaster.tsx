...
const GlasscoProductMaster: React.FC = () => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        const newProducts: Product[] = rawData.map((row, idx) => {
            const commonFields = {
                id: String(row['Item ID'] || `ITM-${Date.now()}-${idx}`),
                company,
  ...
  // Tile KPIs — derived counts across the full product master (no new logic/data)
  const serviceCount   = products.filter(p => p.category === 'Service').length;
  const materialCount  = products.length - serviceCount;
  const categoryCount  = new Set(products.filter(p => p.category !== 'Service').map(p => p.category)).size;
          ...
          return (
              <React.Fragment key={cat}>
                  <tr className="bg-slate-800 text-white no-print">
                      <td colSpan={type === 'materials' ? 8 : 5} className="px-4 py-2 text-2xs font-black uppercase tracking-[0.2em]">{cat}</td>
                  </tr>
                          ...
                          return (
                              <React.Fragment key={gType}>
                                  <tr className="bg-slate-200/80 no-print">
                                      <td colSpan={type === 'materials' ? 8 : 5} className="px-8 py-1.5 text-2xs font-black uppercase text-slate-700 tracking-widest flex items-center">
                                          ...
                                          return (
                                              <React.Fragment key={sub}>
                                                  <tr className="bg-blue-50/50 no-print">
                                                      <td colSpan={type === 'materials' ? 8 : 5} className="px-12 py-1 text-2xs font-black uppercase text-blue-600 tracking-wider">
                                                          {sub} {gType}
                                                      </td>
                                                  </tr>
  ...
  const renderMaterialRow = (p: Product) => {
      ...
      return (
          <tr key={p.id} className="hover:bg-slate-50 transition-colors text-xs">
              ...
              <td className="font-bold text-slate-500 text-2xs uppercase">{p.subCategory || 'Standard'}</td>
              <td className="font-bold text-slate-500 text-2xs uppercase">{p.finishColor || 'N/A'}</td>
              <td className="text-2xs font-bold text-slate-600 uppercase">{p.category === 'Glass' ? (p.sheetSize || 'Standard') : p.unit}</td>
              <td className="text-right">
                  <span className={`text-sm font-black ${stock > 0 ? 'text-emerald-600' : 'text-rose-400'}`}>{formatNumber(Number(stock) || 0)}</span>
                  <span className="text-2xs text-slate-400 ml-1 uppercase">{p.unit}</span>
              </td>
              <td className="pr-4">
  ...
  const renderRateRow = (p: Product) => (
      <tr key={p.id} className="hover:bg-slate-50 transition-colors">
          <td className="font-bold text-2xs uppercase text-slate-700 pl-8 py-2">
            {p.description}
            {(p.priceHistory?.length || 0) > 0 && (
              <span className="ml-1.5 text-2xs font-black text-blue-500 bg-blue-50 px-1 py-0.5 rounded">{p.priceHistory!.length} rev</span>
            )}
          </td>
          <td className="p-1 text-center">
              <input type="number" className="w-20 text-center bg-slate-50 border rounded py-0.5 font-black text-2xs outline-none focus:ring-2 focus:ring-emerald-500" value={p.costPrice || 0} onChange={e => updatePrice(p.id, 'costPrice', Number(e.target.value))} />
          </td>
          <td className="p-1 text-center">
              <input type="number" className="w-20 text-center bg-white border border-blue-200 rounded py-0.5 font-black text-2xs text-blue-700 outline-none focus:ring-2 focus:ring-blue-500" value={p.basePrice || 0} onChange={e => updatePrice(p.id, 'basePrice', Number(e.target.value))} />
          </td>
          <td className="p-1 text-center">
              {p.category === 'Glass' ? (
                <input type="number" className="w-24 text-center bg-emerald-50 border border-emerald-200 rounded py-0.5 font-black text-2xs text-emerald-700 outline-none focus:ring-2 focus:ring-emerald-500" value={p.temperingPrice || 0} onChange={e => updatePrice(p.id, 'temperingPrice', Number(e.target.value))} placeholder="With Temp" />
              ) : (
                <span className="text-2xs text-slate-300">N/A</span>
              )}
          </td>
          <td className="text-center">
              <div className="flex items-center justify-center gap-2">
                {p.basePrice > 0 && <span className={`text-2xs font-black ${p.basePrice > (p.costPrice || 0) ? 'text-emerald-600' : 'text-rose-600'}`}>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      {/* ── KPI row ── */}
      <KpiRow>
        <KpiTile label="Products" value={formatNumber(products.length)} icon={<Package size={16} />} tone="primary" />
        <KpiTile label="Materials" value={formatNumber(materialCount)} icon={<Layers size={16} />} tone="info" hint="raw glass & components" />
        <KpiTile label="Services" value={formatNumber(serviceCount)} icon={<Wrench size={16} />} tone="neutral" hint="processing rates" />
        <KpiTile label="Categories" value={formatNumber(categoryCount)} icon={<FileSpreadsheet size={16} />} tone="warning" hint="material groups" />
      </KpiRow>

      {/* TOOLBAR */}
      <div className="flex flex-col lg:flex-row justify-between items-center bg-white p-3 rounded-card border border-slate-200 shadow-sm w-full no-print gap-4">
        <div className="flex items-center space-x-1">
          <button onClick={() => setActiveTab('materials')} className={`flex items-center space-x-2 px-6 py-2.5 rounded-control font-bold text-xs transition-all ${activeTab === 'materials' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:bg-slate-50'}`}>
        ...
        <div className="flex items-center space-x-3 w-full lg:w-auto overflow-x-auto no-scrollbar pb-1">
           ...
           <div className="flex items-center space-x-1 bg-slate-100 p-1 rounded-control">
           ...
           <div className="h-8 w-px bg-slate-200 hidden lg:block"></div>
               ...
               <div className="relative shrink-0">
                  ...
                  <select
                    className="pl-9 pr-4 py-2 bg-slate-100 border-none rounded-control font-bold text-xs uppercase focus:ring-2 focus:ring-blue-500 outline-none"
                    value={catFilter}
      ...
      {activeTab === 'materials' && (
        <div className="space-y-6 animate-in slide-in-from-bottom-2">
           <div className="bg-slate-900 text-white p-8 rounded-card shadow-xl flex flex-col md:flex-row justify-between items-center relative overflow-hidden">
              ...
              <div className="relative z-10 text-center md:text-left mb-4 md:mb-0">
                 ...
                 <p className="text-2xs font-bold text-slate-400 uppercase tracking-widest mt-1">Registry of raw glass and components</p>
              </div>
              <button onClick={openAddModal} className="bg-white text-slate-900 px-8 py-3 rounded-control font-black uppercase text-xs tracking-widest shadow-xl hover:bg-blue-600 hover:text-white transition-all flex items-center space-x-2 relative z-10">
                 <Plus size={16}/> <span>New Material</span>
              </button>
           </div>

           <div className="bg-white rounded-card border border-slate-200 shadow-sm overflow-hidden min-h-[500px]">
                        ...
                        <tr>
                          <td colSpan={8} className="p-0">
                            <EmptyState
                              ...
                              title={products.length === 0 ? 'No materials yet' : `No matches for "${searchTerm}"`}
                              description={products.length === 0
                                ? 'Add your first raw glass or component to the material library.'
                                : 'Try a different description, ID, or glass type filter.'}
                              action={products.length === 0
                                ? { label: 'New Material', icon: <Plus size={14} />, onClick: openAddModal }
                                : undefined}
      ...
      {activeTab === 'rates' && (
        <div className="bg-white rounded-card border border-slate-200 shadow-sm overflow-hidden animate-in fade-in">
           <div className="p-6 bg-slate-50 border-b flex justify-between items-center">
              <div>
                  <h3 className="font-black text-slate-800 uppercase tracking-tight text-sm">Inventory Pricing Matrix</h3>
                  <p className="text-2xs font-bold text-slate-400 uppercase">Valuation controls for Quotations</p>
              </div>
                         ...
                         <tr>
                           <td colSpan={5} className="p-0">
                             <EmptyState
                               ...
                               title={products.length === 0 ? 'No items to price' : `No matches for "${searchTerm}"`}
                               description={products.length === 0
                                 ? 'Add materials in the Material Library to set their cost, base, and tempering rates here.'
                                 : 'Try a different description, ID, or glass type filter.'}
                               secondaryAction={products.length > 0
                                 ? { label: 'Clear search', onClick: () => setSearchTerm('') }
                                 : undefined}
      ...
      {activeTab === 'services' && (
        <div className="space-y-6">
           <div className="flex justify-between items-center bg-white p-6 rounded-card border border-slate-200 shadow-sm">
              <div className="flex items-center space-x-4">
                  <div className="p-3 bg-indigo-100 text-indigo-700 rounded-card"><Wrench size={24}/></div>
                  <div>
                    <h3 className="font-black uppercase text-slate-800 tracking-tight">Processing Services Registry</h3>
                    <p className="text-2xs font-bold text-slate-400 uppercase">Labor and outsourcing rate control</p>
                  </div>
              </div>
              <button onClick={openAddModal} className="bg-indigo-600 text-white px-8 py-3 rounded-control font-black uppercase text-xs tracking-widest shadow-xl hover:bg-indigo-700 transition-all flex items-center space-x-2"><Plus size={16}/><span>Add Service</span></button>
           </div>
                    ...
                    {filtered.filter(p => p.category === 'Service').map(p => (
                       <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                          <td className="px-6 py-4 font-bold text-xs uppercase text-slate-700">
                              {p.description}
                              {p.brand && (
                                  <span className="ml-2 inline-flex items-center space-x-1 bg-orange-50 text-orange-700 px-2 py-0.5 rounded text-2xs font-black border border-orange-100 uppercase">
                                      <Factory size={10}/> <span>{p.brand}</span>
                                  </span>
                              )}
                          </td>
                          <td><span className="bg-blue-50 text-blue-600 px-2 py-0.5 rounded text-2xs font-black uppercase border border-blue-100">{p.serviceNick || '-'}</span></td>
                          <td><span className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded text-2xs font-black uppercase">{p.thickness || 'All'}</span></td>
                          <td><span className="bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded text-2xs font-black uppercase">{p.unit}</span></td>
                    ...
                    {filtered.filter(p => p.category === 'Service').length === 0 && (
                        <tr>
                          <td colSpan={7} className="p-0">
                            <EmptyState
                              icon={<Wrench size={22} />}
                              title={serviceCount === 0 ? 'No services defined yet' : `No matches for "${searchTerm}"`}
                              description={serviceCount === 0
                                ? 'Add a processing or outsourcing service to control its labour and sales rates.'
                                : 'Try a different service name or clear the search.'}
                              action={serviceCount === 0
                                ? { label: 'Add Service', icon: <Plus size={14} />, onClick: openAddModal }
                                : undefined}
                              secondaryAction={serviceCount > 0
                                ? { label: 'Clear search', onClick: () => setSearchTerm('') }
                                : undefined}
                            />
                          </td>
      ...
      {/* Price History Modal */}
      {priceHistoryProduct && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-modal p-4">
                ...
                <div className="space-y-2">
                  ...
                  {[...(priceHistoryProduct.priceHistory || [])].reverse().map(h => (
                    <div key={h.id} className="grid grid-cols-6 text-[10px] font-bold bg-slate-50 rounded-xl px-3 py-2.5 gap-2 items-center">
                      ...
                      <span className="text-right text-slate-400">{formatNumber(h.oldCostPrice)}</span>
