...
const TagManager: React.FC = () => {
  ...
  const deleteTag = (id: string) => {
    if (!confirm('Delete this tag? It will be removed from all employees.')) return;
      ...
      <div className="flex justify-between items-center bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        <div className="flex items-center space-x-6">
          <div>
            ...
            <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest mt-1.5">{company} Configuration</p>
          </div>
          <div className="h-8 w-px bg-slate-100"></div>
          <div className="flex items-center space-x-1 bg-slate-100 p-1 rounded-xl">
            <button onClick={() => setActiveSection('tags')} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${activeSection === 'tags' ? 'bg-white shadow text-slate-900' : 'text-slate-500'}`}>
              <Tags size={14} className="inline mr-1" />Tags
            </button>
            <button onClick={() => setActiveSection('departments')} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${activeSection === 'departments' ? 'bg-white shadow text-slate-900' : 'text-slate-500'}`}>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{jobTitleTags.length}</span>
              </div>
            ...
            <div className="p-6 flex flex-wrap gap-2">
              ...
              {jobTitleTags.length === 0 && <p className="text-xs text-slate-400 italic">No job title tags yet</p>}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{designationTags.length}</span>
              </div>
            ...
            <div className="p-6 flex flex-wrap gap-2">
              ...
              {designationTags.length === 0 && <p className="text-xs text-slate-400 italic">No designation tags yet</p>}
            </div>
          </div>
        </>
      )}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-slate-50">
            ...
            {departments.length === 0 && (
              <div className="px-6 py-12 text-center text-slate-400 font-medium">No departments configured</div>
            )}
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[500]">
          <div className="bg-white rounded-xl w-full max-w-md shadow-2xl overflow-hidden border border-slate-200">
            ...
            <div className="p-8 space-y-6 bg-slate-50">
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Preview</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Pill Color</label>
                  <input
                    type="color"
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Text Color</label>
                  <input
                    type="color"
      ...
      {isDeptModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[500]">
          <div className="bg-white rounded-xl w-full max-w-md shadow-2xl overflow-hidden border border-slate-200">
            ...
            <div className="p-8 space-y-6 bg-slate-50">
              <div className="space-y-1.5">
                <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Department Name</label>
                <input
                  type="text"
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Parent Department (optional)</label>
                <select
