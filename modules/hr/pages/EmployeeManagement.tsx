...
import { Company, Employee, Account } from '../../shared/types';
...
const EmployeeManagement: React.FC = () => {
  ...
  const initialFormState: Partial<Employee> = {
    ...
    work: { designation: '', department: '', grade: '', joinDate: new Date().toISOString().split('T')[0], employeeCode: '' },
  ...
  const autoCreateLedgerAccount = (employeeName: string) => {
    ...
    // 1. Find suitable L4 Parent (Look for 'Salary', 'Wages', 'Payroll')
    ...
    if (salaryParent) {
      // 2. Check if account already exists for this employee
      ...
      if (!exists) {
        ...
        const newAccount: Account = {
          ...
          company,
          code: newCode,
          name: employeeName, // Use EXACT casing from input
          level: 5,
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      updated = all.map(emp => emp.id === editingId ? { ...(formData as Employee), id: editingId, company } : emp);
      ...
      // Auto-Create Account Only on Creation
          ...
          <table className="w-full text-left min-w-[800px]">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest">Designation & Dept</th>
