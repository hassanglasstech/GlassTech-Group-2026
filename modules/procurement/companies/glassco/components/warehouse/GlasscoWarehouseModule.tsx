...
const GlasscoWarehouseModule: React.FC<{ company: Company }> = ({ company }) => {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-2 rounded-2xl border w-full shadow-sm">
        <div className="flex items-center space-x-1">
          <button onClick={() => setActiveTab('visual')} className={`flex items-center space-x-2 px-6 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === 'visual' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:bg-slate-50'}`}>
        ...
        {activeTab === 'visual' && (
          <div className="relative w-80 mr-2">
             ...
             <input 
               ...
               className="w-full pl-9 pr-4 py-2 bg-slate-100 border-none rounded-xl font-bold text-xs uppercase focus:ring-2 focus:ring-blue-500 outline-none transition-all"
               value={traceSearch}
             ...
             <div key={zone} className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
                   {spots.filter(s => s.zone === zone).map(spot => {
                      ...
                      return (
                        <div key={spot.id} className={`rounded-3xl border p-6 flex flex-col items-center justify-center text-center space-y-3 transition-all group relative overflow-hidden ${highlighted ? 'bg-blue-600 border-blue-600 shadow-xl ring-4 ring-blue-200' : 'bg-white border-slate-200 hover:shadow-xl'}`}>
                           <div className={`p-3 rounded-2xl transition-all ${highlighted ? 'bg-white text-blue-600' : 'bg-blue-50 text-blue-600 group-hover:bg-blue-600 group-hover:text-white'}`}>
                              <Box size={24}/>
                           </div>
                           <div>
                              <p className={`font-black leading-none ${highlighted ? 'text-white' : 'text-slate-900'}`}>{spot.code}</p>
                              <p className={`text-[10px] font-bold mt-1 uppercase tracking-widest ${highlighted ? 'text-blue-100' : 'text-slate-400'}`}>{spotPieces.length} Items</p>
                           </div>
...
export default GlasscoWarehouseModule;
