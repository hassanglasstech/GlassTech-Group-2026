...
const POPrint: React.FC<{ po: PurchaseOrder; onClose: () => void }> = ({ po, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/80 flex items-start justify-center z-[600] overflow-y-auto py-6 px-4">
        ...
        <div className="p-10 text-black" style={{ fontFamily: 'Arial, sans-serif', minHeight: '257mm' }}>
          ...
          <div className="flex justify-between items-start pb-5 mb-6"
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: 11, color: '#64748b', marginTop: 3, fontWeight: 700 }}>Date: {po.date}</div>
              {poMeta.deliveryDate && (
                <div style={{ fontSize: 11, color: '#dc2626', fontWeight: 700, marginTop: 2 }}>
                  Delivery By: {poMeta.deliveryDate}
                </div>
              )}
            </div>
          </div>
              ...
              {items.map((item, i) => (
                <tr key={i} style={{ background: i % 2 === 0 ? '#fff' : '#f8fafc', borderBottom: '0.5px solid #e2e8f0' }}>
                  ...
                  <td style={{ padding: '7px 8px', textAlign: 'right', color: '#1d4ed8' }}>
                    PKR {(item.meta.freightPKR || 0).toLocaleString()}
                  </td>
                  <td style={{ padding: '7px 8px', textAlign: 'right', fontWeight: 900, color: '#059669' }}>
                    PKR {Math.round(item.meta.lineTotal || 0).toLocaleString()}
                  </td>
                </tr>
              ))}
            </tbody>
                    ...
                    return (
                      <React.Fragment key={po.id}>
                        <tr className="border-b hover:bg-slate-50 cursor-pointer"
                          ...
                          <td className="px-4 py-3">
                            <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${STATUS_CLS[po.status] || 'bg-slate-100 text-slate-600'}`}>
                              {po.status}
                            </span>
                          </td>
                          <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                            <div className="flex gap-1.5">
                              <button onClick={() => setPrintPO(po)}
                                className="flex items-center gap-1 text-[10px] font-bold text-slate-400 hover:text-blue-600 border border-slate-200 hover:border-blue-200 px-2 py-1 rounded-lg">
                        ...
                        {isExp && (
                          <tr>
                            <td colSpan={8} className="bg-slate-50 border-b px-6 py-4">
                              <p className="text-[9px] font-black uppercase text-slate-400 mb-2 tracking-widest">Line Items</p>
                              <table className="w-full text-xs">
                                ...
                                <tbody className="divide-y divide-slate-100">
                                  {poItems.map((item, i) => (
                                    <tr key={i}>
                                      ...
                                      <td className="py-1.5 text-right">PKR {(item.rate || 0).toLocaleString()}</td>
                                      <td className="py-1.5 text-right text-blue-600">PKR {(item.m.freightPKR || 0).toLocaleString()}</td>
                                      <td className="py-1.5 text-right font-black text-emerald-700">
                                        PKR {Math.round(item.m.lineTotal || 0).toLocaleString()}
                                      </td>
                                    </tr>
                                  ))}
                                </tbody>
                                <tfoot className="border-t-2 border-slate-300">
                                  <tr>
                                    <td colSpan={5} className="pt-2 text-right text-[10px] font-black text-slate-500 uppercase">Total</td>
                              ...
                              {(po as any).headerRemarks && (
                                <p className="mt-2 text-[10px] text-slate-500 italic">{(po as any).headerRemarks}</p>
                              )}
                            </td>
                          </tr>
                        )}
      ...
      {view === 'create' && (
        <div className="space-y-5">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-1.5">
                ...
                {glassVendors.length === 0 && (
                  <p className="text-[9px] text-amber-600 font-bold">No Glass vendors. Add in Vendor Hub (type = Glass)</p>
                )}
              </div>
              <div className="space-y-1.5">
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-7">
            ...
            {/* Column labels */}
            <div className="grid text-[9px] font-black uppercase text-slate-400 mb-2 px-2 gap-2"
                  ...
                  <div key={line.id}
                    ...
                    <div className="p-3 grid gap-2 items-start"
                      ...
                      <div className="relative">
                        <div className="flex items-center gap-1.5 mb-1 min-h-[18px]">
                          ...
                          {line.thickness && (
                            <span className="text-[9px] font-black text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded leading-none">
                              {line.category} · {line.thickness} · {line.sheetSize}"
                            </span>
                          )}
                          {line.stockOnHand > 0 && (
                            <span className="text-[9px] font-bold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded leading-none">
                            ...
                            {suggs.length === 0 ? (
                              ...
                              <button key={s.key}
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${s.source === 'master' ? 'bg-blue-50 text-blue-600' : 'bg-emerald-50 text-emerald-600'}`}>
                                    {s.source === 'master' ? 'Product Master' : 'GRN History'}
                                  </span>
                                </div>
                                <div className="text-right text-[9px] shrink-0 ml-4">
                      ...
                      <div className="mt-[18px]">
                        {line.lastMAP > 0 && (
                          <div className="text-[8px] font-bold text-emerald-600 text-right mb-0.5 leading-none">
                      ...
                      <div className={`text-sm font-black text-right pr-1 mt-[18px] ${line.lineTotal > 0 ? 'text-emerald-700' : 'text-slate-200'}`}>
                        {line.lineTotal > 0 ? Math.round(line.lineTotal).toLocaleString() : '—'}
                      </div>
                    ...
                    {isFilled && (
                      <div className="px-3 pb-3 flex items-center gap-2">
                        ...
                        <input type="text"
                          className="sap-input text-[11px] flex-1 py-1"
            ...
            <div className="mt-5 pt-4 border-t">
              <div className="grid grid-cols-4 gap-3 mb-3">
                {[
                  ...
                  { label: 'Total Sheets', val: totalSheets.toLocaleString() },
                  { label: 'Total SqFt', val: totalSqft.toFixed(1) },
                  { label: 'Total Freight', val: `PKR ${totalFreight.toLocaleString()}` },
                ].map(s => (
