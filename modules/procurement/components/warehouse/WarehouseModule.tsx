// Unified Warehouse Module — replaces GlasscoWarehouseModule + NipponWarehouseModule
...
const WarehouseModule: React.FC<{ company: Company }> = ({ company }) => {
  ...
  // Company-aware theming
  const accent = company === 'Nippon' ? 'red' : 'blue';
  const cls = {
    tabActive: accent === 'red' ? 'bg-red-600 text-white' : 'bg-blue-600 text-white',
    ring: accent === 'red' ? 'focus:ring-red-500' : 'focus:ring-blue-500',
    spotHL: accent === 'red' ? 'bg-red-600 border-red-600 ring-4 ring-red-200' : 'bg-blue-600 border-blue-600 ring-4 ring-blue-200',
    spotIcon: accent === 'red' ? 'bg-white text-red-600' : 'bg-white text-blue-600',
    spotIconIdle: accent === 'red' ? 'bg-red-50 text-red-600 group-hover:bg-red-600 group-hover:text-white' : 'bg-blue-50 text-blue-600 group-hover:bg-blue-600 group-hover:text-white',
    hlOverlay: accent === 'red' ? 'bg-red-400/20' : 'bg-blue-400/20',
    hlText: accent === 'red' ? 'text-red-100' : 'text-blue-100',
  };
  ...
  const handleSaveSpot = () => {
    ...
    const spot: WarehouseSpot = {
      id: `SPOT-${Date.now()}`, company,
      code: newSpot.code.toUpperCase(), zone: newSpot.zone as any
    };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-2 rounded-2xl border w-full shadow-sm">
        <div className="flex items-center space-x-1">
          <button onClick={() => setActiveTab('visual')} className={`flex items-center space-x-2 px-6 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === 'visual' ? cls.tabActive : 'text-slate-500 hover:bg-slate-50'}`}>
            <LayoutGrid size={16} /><span>Warehouse Map</span>
          </button>
          <button onClick={() => setActiveTab('settings')} className={`flex items-center space-x-2 px-6 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === 'settings' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:bg-slate-50'}`}>
        ...
        {activeTab === 'visual' && (
          <div className="relative w-80 mr-2">
             ...
             <input type="text" placeholder="Trace Piece ID..." 
               className={`w-full pl-9 pr-4 py-2 bg-slate-100 border-none rounded-xl font-bold text-xs uppercase focus:ring-2 ${cls.ring} outline-none transition-all`}
               value={traceSearch} onChange={e => setTraceSearch(e.target.value)} />
          </div>
        )}
      </div>
             ...
             <div key={zone} className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
                   {spots.filter(s => s.zone === zone).map(spot => {
                      ...
                      return (
                        <div key={spot.id} className={`rounded-3xl border p-6 flex flex-col items-center justify-center text-center space-y-3 transition-all group relative overflow-hidden ${highlighted ? cls.spotHL : 'bg-white border-slate-200 hover:shadow-xl'}`}>
                           <div className={`p-3 rounded-2xl transition-all ${highlighted ? cls.spotIcon : cls.spotIconIdle}`}>
                              <Box size={24}/>
                           </div>
                           <div>
                              <p className={`font-black leading-none ${highlighted ? 'text-white' : 'text-slate-900'}`}>{spot.code}</p>
                              <p className={`text-[10px] font-bold mt-1 uppercase tracking-widest ${highlighted ? cls.hlText : 'text-slate-400'}`}>{spotPieces.length} Items</p>
                           </div>
                           ...
                           {highlighted && (
                             <div className={`absolute inset-0 ${cls.hlOverlay} animate-pulse pointer-events-none`}></div>
                           )}
                        </div>
                      );
                   })}
           ...
           <div className="bg-white rounded-3xl border shadow-sm overflow-hidden">
              <table className="w-full text-left sap-table">
                 ...
                 <tbody>
                    {spots.map(spot => (
                      <tr key={spot.id}>
                         <td className="font-black text-indigo-600">{spot.code}</td>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[400]">
           <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in duration-200 border">
              ...
              <div className="px-10 py-6 bg-white border-t flex justify-end space-x-4">
                 ...
                 <button onClick={handleSaveSpot} className="bg-indigo-600 text-white px-10 py-3 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl">Define Spot</button>
              </div>
           </div>
        </div>
      )}
