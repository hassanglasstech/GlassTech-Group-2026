...
- ✅ **D3 → Recut → supervisor pool — SHIPPED (`99ae574`):** failing a piece at QC (disposal Recut) now clears its per-piece `assignedCutter` ('' = pool), keeps the failing cutter in `prevCutters[]` + appends the fault to `faultHistory[]` + sets `blockedReason`. CutterWorkbench gets a privileged **"Recut Pool"** section to reassign each rejected piece to a cutter (reuses `reassignRemainingPieces`); the cutter's queue then shows it with a RECUT badge and re-cuts it via the already-allowed `QC-Failed → Cut`. Never auto-returns to the original cutter. Migration-free (only allowed transitions). tsc 0 · 369 tests · build clean. **Auth-gated — preview-test on GT-Production.**
